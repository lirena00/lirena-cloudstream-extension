...
class KronchHIProvider: MainAPI() {
   ...
   /* private suspend fun proxyToken(): Map<String, String> {
    ...
    private suspend fun getMovie(id: String?):ArrayList<Episode> {
        ...
        val metadainfo = app.get("$krunchyapi/content/v2/cms/movie_listings/$id/movies?locale=hi-IN", headers = latestKrunchyHeader).parsed<KrunchyLoadMain>()
        metadainfo.data.map {
            val title = it.title
            val epID = it.id
    ...
    private suspend fun getRecommendations(seriesId: String?): List<SearchResponse>?{
        proxyToken()
        val recsurl = "$krunchyapi/content/v2/discover/similar_to/$seriesId?locale=hi-IN&n=30"
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tttt = if (isMovie) "$krunchyapi/content/v2/cms/movie_listings/$seriesIDSuper?locale=hi-IN" else "$krunchyapi/content/v2/cms/series/$seriesIDSuper?locale=hi-IN"
        ...
        if (tvType == TvType.Anime) {
            ...
            val nn =
                app.get(
                    "${krunchyapi}/content/v2/cms/series/$seriesIDSuper/seasons?locale=hi-IN",
                    headers = latestKrunchyHeader
                )
            ...
            inn.apmap { main ->
                ...
                val res =
                    app.get(
                        "${krunchyapi}/content/v2/cms/seasons/$mainID/episodes?&locale=hi-IN",
                        headers = latestKrunchyHeader
                    )
                        .parsedSafe<BetaKronch>()
                        ?: throw ErrorLoadingException("Couldn't get episodes, try again")
            ...
            innversions.map { ve ->
                val versionsfiltered =
                    ve.versions?.filter {
                        (it.audioLocale?.contains(Regex("ja-JP|zh-CN|hi-IN")) == true ||
                ...
                versionsfiltered?.apmap { vers ->
                    ...
                    val resv =
                        app.get(
                            "${krunchyapi}/content/v2/cms/seasons/$guid/episodes?&locale=hi-IN",
                            headers = latestKrunchyHeader
                        )
                            .parsedSafe<BetaKronch>()
                            ?: throw ErrorLoadingException("Couldn't get episodes, try again")
